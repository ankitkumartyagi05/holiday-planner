const app = {
    state: {
        step: 'home', country: null, destination: null, accommodation: null, transport: null,
    ...
    data: {
        ...
        // Rich Data with specific images for better visuals
        destinations: {
            'IN': [
                { id: 1, name: 'Goa Beach', type: 'Beach', cost: 3000, img: 'https://picsum.photos/seed/goabeach/400/300' },
                { id: 2, name: 'Manali Hills', type: 'Mountains', cost: 4500, img: 'https://picsum.photos/seed/manalihills/400/300' },
                { id: 3, name: 'Mumbai City', type: 'City', cost: 5000, img: 'https://picsum.photos/seed/mumbaicity/400/300' }
            ],
            'US': [
                { id: 4, name: 'Miami Coast', type: 'Beach', cost: 200, img: 'https://picsum.photos/seed/miamibeach/400/300' },
                { id: 5, name: 'Denver Ranges', type: 'Mountains', cost: 180, img: 'https://picsum.photos/seed/denvermtns/400/300' },
                { id: 6, name: 'New York', type: 'City', cost: 300, img: 'https://picsum.photos/seed/nycity/400/300' }
            ],
            'GB': [
                { id: 7, name: 'Cornwall', type: 'Beach', cost: 120, img: 'https://picsum.photos/seed/cornwallbeach/400/300' },
                { id: 8, name: 'Scottish Highlands', type: 'Mountains', cost: 110, img: 'https://picsum.photos/seed/scottishhills/400/300' },
                { id: 9, name: 'London', type: 'City', cost: 200, img: 'https://picsum.photos/seed/londoncity/400/300' }
            ],
            'FR': [
                { id: 10, name: 'Nice Riviera', type: 'Beach', cost: 140, img: 'https://picsum.photos/seed/nicebeach/400/300' },
                { id: 11, name: 'Chamonix', type: 'Mountains', cost: 130, img: 'https://picsum.photos/seed/chamonix/400/300' },
                { id: 12, name: 'Paris', type: 'City', cost: 180, img: 'https://picsum.photos/seed/pariscity/400/300' }
            ],
            'AE': [
                { id: 13, name: 'JBR Beach', type: 'Beach', cost: 150, img: 'https://picsum.photos/seed/jbrdubai/400/300' },
                { id: 14, name: 'Hatta Mountains', type: 'Mountains', cost: 100, img: 'https://picsum.photos/seed/hatta/400/300' },
                { id: 15, name: 'Dubai Marina', type: 'City', cost: 220, img: 'https://picsum.photos/seed/dubaimarina/400/300' }
            ]
        },
        hotels: [{ name: 'Backpacker Hostel', mult: 0.5 }, { name: '3-Star Business Hotel', mult: 1.0 }, { name: '5-Star Luxury Resort', mult: 3.0 }],
        transport: [{ name: 'Public Bus', cost: 10 }, { name: 'Metro / Train', cost: 20 }, { name: 'Car Rental', cost: 80 }, { name: 'Private Flight', cost: 150 }]
    },
    init: function () {
        $('.view-section').hide();
        $('#view-home').show();
        ...
        const html = this.data.countries.map(c => `
                    <div class="col-4 col-md-2">
                        <div class="card text-center p-2 cursor-pointer shadow-sm h-100 d-flex flex-column justify-content-center" onclick="app.setCountry('${c.code}')">
                            <div class="fs-2 mb-1">${c.flag}</div>
                            <div class="small fw-bold text-truncate">${c.name}</div>
                            <div class="small text-muted" style="font-size:0.7rem;">${c.currency}</div>
                        </div>
                    </div>
                `).join('');
    ...
    renderDestinations: function () {
        ...
        const html = list.map(d => `
                    <div class="col-md-4 col-sm-6 dest-card theme-${d.type}">
                        <div class="card h-100">
                            ...
                            <div class="card-body">
                                ...
                                <button class="btn btn-outline-primary w-100 mt-2" onclick="app.setDest(${d.id})">Select (${this.formatMoney(d.cost)})</button>
                            </div>
                        </div>
                    </div>
                `).join('');
    ...
    filterDest: function (type) {
        ...
        else { $('.dest-card').hide(); $(`.theme-${type}`).fadeIn(); }
    },
    setDest: function (id) {
        const list = this.data.destinations[this.state.country.code];
    ...
    renderStayTransport: function () {
        ...
        $('#accomList').html(this.data.hotels.map((h, i) =>
            `<li class="list-group-item list-group-item-action d-flex justify-content-between align-items-center cursor-pointer" onclick="app.setHotel(${i}, ${destCost * h.mult})">
        ...
        $('#transportList').html(this.data.transport.map((t, i) =>
            `<li class="list-group-item list-group-item-action d-flex justify-content-between align-items-center cursor-pointer" onclick="app.setTrans(${i}, ${t.cost * this.state.country.rate})">
        ...
        const html = this.state.itinerary.map((day, i) => `
                    <div class="col-md-6 col-lg-4">
                        <div class="card h-100 shadow-sm">
                            <div class="card-header bg-light fw-bold">Day ${i + 1} - ${day.date}</div>
                            <div class="card-body" id="acts-${i}">
                                ${day.acts.length ? '' : '<p class="text-muted small text-center">No activities planned</p>'}
                            </div>
                            <div class="card-footer text-center bg-white"><button class="btn btn-sm btn-outline-primary w-100" onclick="app.addAct(${i})">+ Add Activity</button></div>
                        </div>
                    </div>
                `).join('');
        ...
        this.state.itinerary.forEach((day, i) => {
            const html = day.acts.map((a, j) => `
                        <div class="d-flex justify-content-between border-bottom py-2 small">
                            <div><span class="badge bg-secondary me-2">${a.time}</span>${a.desc}</div>
                            <div>
                                <span class="text-muted me-2">${this.formatMoney(a.cost)}</span> 
    ...
    renderReview: function () {
        ...
        $('#reviewDetailsTable').html(`
                    ...
                    <tr><td>Duration</td><td class="text-end">${totalDays} Days</td></tr>
                `);
        $('#budgetList').html(`
